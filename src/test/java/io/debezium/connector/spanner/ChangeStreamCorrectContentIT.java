...
public class ChangeStreamCorrectContentIT extends AbstractSpannerConnectorIT {
    ...
    public void shouldCarryAllPrimaryKeyColumnsInKeyStruct(PartitionMode partitionMode) throws Exception {
        assumeSupportedPartitionMode(partitionMode);
    ...
    // The Spanner emulator does not propagate transaction tags through its change
    // stream: confirmed by inspecting the raw DataChangeEvent parsed straight from
    // the emulator's stream, which already shows an empty transactionTag before any
    // connector processing happens. This scenario needs a real fully featured Spanner
    // instance to validate.
    @Disabled("Transaction tags are not propagated through change streams by the Spanner emulator; requires a real fully featured Spanner instance")
    @Test
