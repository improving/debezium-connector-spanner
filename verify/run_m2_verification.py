...
Milestone 2 End-to-End Verification Harness for Cloud Spanner Kafka Connector:
  1. Completeness & Zero Data Loss Verification
  2. Field-Level Value Fidelity Verification (INT64, NUMERIC, JSON, TIMESTAMP,
  BOOL, BYTES)
  3. Strict Per-Key Monotonic Ordering Verification
  4. End-to-End Replication Latency SLA Metrics

Location: verify/run_m2_verification.py
Proposal: verification_guide.md#milestone-2-m2-rich-schema-fidelity-monotonic-ordering--latency-slas
"""
...
def execute_milestone2_verification(
  ...
  print("\n" + "=" * 80)
  print("  CLOUD SPANNER KAFKA CONNECTOR - MILESTONE 2 VERIFICATION HARNESS")
  print(
      "  (Field-Level Value Fidelity, Per-Key Ordering, Completeness, & SLA"
      " Latency)"
  )
  print("=" * 80)
  ...
  print(
      f"  Target Table    : {table} (Rich Schema: INT64, NUMERIC, JSON,"
      " TIMESTAMP, BOOL, BYTES)"
  )
  ...
  # Step 2: Run GCSB Rich Schema Workload Driver
  ...
  print(
      f"\nStep 2/5: Executing GCSB {table} Rich Schema Mixed Workload Driver..."
  )
  try:
  ...
  # Step 4: Provision BigQuery External Tables with Rich Schemas
  print(
...
"""
  ...
  # Step 5: Execute Milestone 2 Comprehensive Audit Queries
  print(
      "\nStep 5/5: Executing Milestone 2 Verification Queries (Fidelity,"
      " Ordering, Completeness, SLA)..."
  )
...
ValueFidelityMismatches AS (
  ...
  WHERE src.kafka_op IN ('c', 'u')
    AND (
      (src.user_name IS NOT NULL AND src.user_name != JSON_VALUE(k.payload.after.UserName))
...
"""
  ...
  print("-" * 80)
  print(f"  1. Completeness (Missing Rows)  : {missing_cnt:,}  [PASS: 0]")
  print(
      f"  2. Field-Level Value Fidelity   : {value_mismatches_cnt:,} Mismatches"
      "  [PASS: 0]"
  )
  print(f"     - Checked Fields: INT64, NUMERIC, JSON, TIMESTAMP, BOOL, BYTES")
  print(
      f"  3. Strict Per-Key Ordering      : {ordering_violations_cnt:,}"
      " Violations  [PASS: 0]"
  )
  print(
  ...
  if passed:
    print(
        "  VERIFICATION STATUS             : [PASS] MILESTONE 2 FULL COMPLIANCE"
        " CONFIRMED!"
    )
    print("=" * 80 + "\n")
    return True
    ...
    print(
        f"  VERIFICATION STATUS             : [FAIL] M1/M2 COMPLIANCE CRITERIA"
        f" NOT MET."
    )
    print("=" * 80 + "\n")
    return False
...
if __name__ == "__main__":
  parser = argparse.ArgumentParser(
      description=(
          "Cloud Spanner Kafka Connector Milestone 2 Verification Orchestrator"
      )
  )
  ...
  success = execute_milestone2_verification(
