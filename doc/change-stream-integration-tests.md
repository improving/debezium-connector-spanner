...
- **Spanner Omni Docker Container** - a Docker-based fallback backend for `MUTABLE_KEY_RANGE`
  ...
  where relevant.
- **A real, multi-region Spanner instance** (`-Preal-spanner`) - available, and the preferred
  backend for `MUTABLE_KEY_RANGE` tests going forward; several key scenarios (the schema-change
  tests and the forced key-range-split tests) have been independently confirmed passing against
  it.
...
**Runs against:** a real Cloud Spanner instance only (`-Preal-spanner`) - gated behind
`@EnabledIf("hasNonEmulatorBackend")`. Requires the pre-provisioned `east-partition`/
`west-partition` instance partitions described in
...
## Blocked / disabled - interleaved children of a placement table

Two classes below are `@EnabledIf("hasNonEmulatorBackend")` (not `@Disabled` - they're
expected to attempt a real run), but neither can currently complete against any available
backend. Unlike `PlacementKeyMoveIT` above, this isn't the (now-resolved) geo-partitioning
platform restriction - it's a distinct issue specific to a change stream over a
placement-key parent table *with an interleaved child table*: the change stream is created
successfully, but when the connector tries to actually stream from it, Spanner returns
`INVALID_ARGUMENT: Table-valued function not found: READ_<streamname>`, and the connector
retries this indefinitely with no backoff or give-up condition. Confirmed reproducible
(~146,000 identical retries logged in ~20 minutes via `CascadingDeleteDuringPlacementMoveIT`
before the run was killed by hand); not yet root-caused. The local emulator and Spanner Omni
remain blocked for the same reasons as `PlacementKeyMoveIT` (DDL rejection and disabled
geo-partitioning, respectively).

**Runs against:** none currently, for the reason above. `InterleavedPlacementMoveIT` and
`CascadingDeleteDuringPlacementMoveIT` are both real, compiling tests that reuse the existing
`Connection`/`AbstractSpannerConnectorIT` API, so the compiler type-checks them on every
build - they just can't complete a run yet.
...
indirectly through correct ordering. Blocked on the TVF gap above.

**`CascadingDeleteDuringPlacementMoveIT.shouldOrderCascadingDeleteCorrectlyRelativeToInFlightPlacementMove`**
Would insert a parent row with an interleaved child (using `ON DELETE CASCADE`), move the
parent to a different placement, then immediately delete the parent so the delete cascades
to the child. Would confirm both parent and child produce the expected
...
same row's disappearance at once. Blocked on the TVF gap above - this is the test that
surfaced it.
